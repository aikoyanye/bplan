...
import com.zjh.store.GoodsDetailAttrBean;

import java.util.List;

public class SkuAdapter extends BaseAdapter{
	private List<GoodsDetailAttrBean> list;//数据源
	...
	public SkuAdapter(List<GoodsDetailAttrBean> list, Context context) {
		super();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mInflater.inflate(R.layout.goods_detail_attr_item, null);
			/** 得到各个控件的对象 */
			holder.title = (TextView) convertView.findViewById(R.id.ItemText);
			holder.layout = (LinearLayout) convertView.findViewById(R.id.attr_layout);
			convertView.setTag(holder);// 绑定ViewHolder对象
		} else {
			holder = (ViewHolder) convertView.getTag();// 取出ViewHolder对象
		}
		final GoodsDetailAttrBean bean = list.get(position);
